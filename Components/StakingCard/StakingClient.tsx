...
export default function useStakingClient({ onLog }: StakingClientProps) {
  ...
  const stakeWithInit = async (stakeParams: StakeParams) => {
    ...
    try {
      ...
      const txHash = await sendTransaction(tx, connection);
      addLog(`Stake initialized and tokens staked successfully! TX: ${txHash}`, 'success');
      addLog(`User State: ${userState.toString()}`, 'info');
  ...
  const getUserState = async (poolAddress: string, userAddress: string, randomSeed: number[]) => {
    try {
  ...
  const getPoolTokenBalance = async (poolAddress: string) => {
    try {
      ...
      const accountData = await connection.getTokenAccountBalance(poolTokensAccount);
