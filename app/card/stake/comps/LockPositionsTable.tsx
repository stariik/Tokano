...
interface LockPositionsTableProps {
  lockState?: any; // expected to include tokenInfo and lock fields
  onClaimed?: () => void;
}

const LockPositionsTable: React.FC<LockPositionsTableProps> = ({
  lockState,
  onClaimed,
}) => {
    ...
    <div>
      <div className="font-khand dark:border-secondary mt-6 overflow-hidden rounded-4xl border-2 border-[#CDCDE9]">
          ...
          {!publicKey ? (
            ...
            <table className="h-full w-full">
              <tbody className="h-full text-center">
                <tr
                  onClick={() => {
                    // open claim popup when user owns this lock and there's something claimable
        ...
        <div className="dark:border-secondary border-t border-[#CDCDE9] px-6 py-3 text-center text-sm dark:bg-[#0A0520] dark:text-white">
          This table shows the lock attached to this page if it belongs to your
          connected wallet.
        </div>
      </div>
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    transactionListener(connection, txId, (completed) => {
                      ...
                      setClaimOpen(false);
                      if (completed) {
                        alert("Lock claim completed");
                      } else {
                        alert("Lock claim failed");
                      }
                      onClaimed?.();
                    });
                  } catch (err) {
...
export default LockPositionsTable;
