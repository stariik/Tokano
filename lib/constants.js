...
export const FORM_CONFIGS = {
  "STAKING POOL": {
    fields: [
      { name: "activationDate", type: "date", label: "Activation Date", required: true },
      { name: "activationTime", type: "time", label: "Activation Time", required: true },
      { name: "rewardAmount", type: "number", label: "Reward Amount", placeholder: "0", suffix: "tokens" },
      { name: "distributionLength", type: "number", label: "Distribution Length", defaultValue: 100, suffix: "days" },
      { name: "unstakingPeriod", type: "number", label: "Unstaking Period", placeholder: "0", suffix: "days" },
    ],
  ...
  "LOCK FUNDS": {
    fields: [
      ...
      { name: "tokenAmount", type: "number", label: "Token Amount", placeholder: "0", suffix: "tokens" },
      { name: "releaseDate", type: "datetime-local", label: "Release Date", required: true },
      { name: "recipientWallet", type: "text", label: "Recipient Wallet", placeholder: "Enter wallet address" },
    ],
    description: "tokens get locked on",
    attentionMessage: "tokens will not be transferred automatically and need to be claimed by owner."
  },
  "VEST FUNDS": {
    fields: [
      { name: "activationDate", type: "date", label: "Activation Date", required: true },
      { name: "activationTime", type: "time", label: "Activation Time", required: true },
      { name: "tokenAmount", type: "number", label: "Token Amount", placeholder: "0", suffix: "tokens" },
      { name: "cliffPeriod", type: "number", label: "Cliff Period", defaultValue: 100, suffix: "days" },
      { name: "releaseModel", type: "select", label: "Release Model", options: ["monthly", "weekly", "daily"], defaultValue: "monthly" },
      { name: "recipientWallet", type: "text", label: "Recipient Wallet", placeholder: "Enter wallet address" },
    ],
...
export const getFundTypeById = (id) => FUND_TYPES.find(fund => fund.id === id);
